...
#curl -X POST "https://git-central.openfunction.co/api/v1/repos/jmarhee/pyputio-cli/releases" -H "Authorization: token $AUTH_TOKEN" \
...
curl \
  -X POST "https://api.github.com/repos/jmarhee/terraform-digitalocean-ghost-docker/releases" -H "Authorization: token $GITHUB_TOKEN" \
